...
require_once 'function/f_kelulusan.php';

$api_name = 'api_kelulusan';
...
$fn_kelulusan = new Class_kelulusan();

try {
    $fn_general->__set('constant', $constant);
    $fn_login->__set('constant', $constant);
    $fn_login->__set('fn_general', $fn_general);
    $fn_kelulusan->__set('constant', $constant);
    $fn_kelulusan->__set('fn_general', $fn_general);
    ...
    if ('GET' === $request_method) {
        ...
        if (!is_null($kelulusanId)) {
            $fn_kelulusan->__set('kelulusanId', $kelulusanId);
            $result = $fn_kelulusan->get_kelulusan();
        } else {
            $result = $fn_kelulusan->get_kelulusan_list();
        }
        ...
        $fn_kelulusan->__set('userId', $jwt_data->userId);
        $result = $fn_kelulusan->add_kelulusan($params);
        $fn_general->updateVersion(16);
        $fn_general->save_audit(33, $jwt_data->userId, 'Kod = ' . $kod);
        ...
        $fn_kelulusan->__set('kelulusanId', $kelulusanId);
        $fn_kelulusan->update_kelulusan($put_vars);
        $fn_general->updateVersion(16);
        $fn_general->save_audit(34, $jwt_data->userId, 'Kod = ' . $put_vars['kod']);
