...
class Class_peringkat {
     ...
     * @throws Exception
     */
    public function get_peringkat_list () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            $result = array();
            $dbResults = Class_db::getInstance()->db_select('ref_peringkat');
            foreach ($dbResults as $dbResult) {
                $rowResult['peringkatId'] = $dbResult['PERINGKAT_ID'];
    ...
    public function get_peringkat () {
        try {
            ...
            if (empty($this->peringkatId)) {
                throw new Exception('[' . __LINE__ . '] - Parameter peringkatId empty');
            }

            $result = array();
            $dbResult = Class_db::getInstance()->db_select_single('ref_peringkat', array('PERINGKAT_ID'=>$this->peringkatId), null, 1);
            $result['peringkatId'] = $dbResult['PERINGKAT_ID'];
     ...
     * @throws Exception
     */
    public function add_peringkat ($params=array()) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            if (Class_db::getInstance()->db_count('ref_peringkat', array('KOD'=>$kod)) > 0) {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_PERINGKAT_EXIST, 31);
            }

            $user = Class_db::getInstance()->db_select_single('sys_user', array('user_id'=>$this->userId), null, 1);
            return Class_db::getInstance()->db_insert('ref_peringkat', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'KATEGORI'=>$kategori, 'JANTINA'=>$jantina, 'NILAI'=>$nilai, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'GAB_YT'=>$gabYt, 'SAH_YT'=>$sahYt,
     ...
     * @throws Exception
     */
    public function update_peringkat ($putVars) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $constant = $this->constant;
            ...
            Class_db::getInstance()->db_update('ref_peringkat', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'KATEGORI'=>$kategori, 'JANTINA'=>$jantina, 'NILAI'=>$nilai, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'GAB_YT'=>$gabYt, 'SAH_YT'=>$sahYt,  'TARIKH_UBAHSUAI'=>'Now()'), array('PERINGKAT_ID'=>$this->peringkatId));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
