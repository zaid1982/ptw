...
class Class_paper_julai {
     ...
     * @throws Exception
     */
    public function get_paper_julai_list () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            $result = array();
            $dbResults = Class_db::getInstance()->db_select('ref_paper_julai');
     ...
     * @throws Exception
     */
    public function get_paper_julai () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            $result = array();
            $dbResult = Class_db::getInstance()->db_select_single('ref_paper_julai', array('mpel_kod'=>$this->mpelKod), null, 1);
     ...
     * @throws Exception
     */
    public function add_paper_julai ($params=array()) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            if (Class_db::getInstance()->db_count('ref_paper_julai', array('mpel_kod'=>$mpelKod)) > 0) {
            ...
            Class_db::getInstance()->db_insert('ref_paper_julai', array('mpel_kod'=>$mpelKod, 'diskripsi'=>$diskripsi, 'sah_yt'=>$sahYt,
     ...
     * @throws Exception
     */
    public function update_paper_julai ($putVars) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $constant = $this->constant;
            ...
            Class_db::getInstance()->db_update('ref_paper_julai', array('diskripsi'=>$diskripsi, 'sah_yt'=>$sahYt,  'd_ubahsuai'=>'Now()'), array('mpel_kod'=>$this->mpelKod));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
