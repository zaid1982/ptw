...
class Class_kecacatan_calon {

    private $constant;
    private $fn_general;
    private $kecacatanCalonId;
    private $userId;
     ...
     * @throws Exception
     */
    public function get_kecacatan_calon_list () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            $result = array();
            $dbResults = Class_db::getInstance()->db_select('ref_kecacatan_calon');
            foreach ($dbResults as $dbResult) {
                $rowResult['kecacatanCalonId'] = $dbResult['KECACATAN_CALON_ID'];
     ...
     * @throws Exception
     */
    public function get_kecacatan_calon () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            if (empty($this->kecacatanCalonId)) {
                throw new Exception('[' . __LINE__ . '] - Parameter kecacatanCalonId empty');
            }

            $result = array();
            $dbResult = Class_db::getInstance()->db_select_single('ref_kecacatan_calon', array('KECACATAN_CALON_ID'=>$this->kecacatanCalonId), null, 1);
            $result['kecacatanCalonId'] = $dbResult['KECACATAN_CALON_ID'];
     ...
     * @throws Exception
     */
    public function add_kecacatan_calon ($params=array()) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            if (Class_db::getInstance()->db_count('ref_kecacatan_calon', array('KOD'=>$kod)) > 0) {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_KECACATAN_CALON_EXIST, 31);
            }

            $user = Class_db::getInstance()->db_select_single('sys_user', array('user_id'=>$this->userId), null, 1);
            return Class_db::getInstance()->db_insert('ref_kecacatan_calon', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'KATEGORI'=>$kategori, 'JANTINA'=>$jantina, 'NILAI'=>$nilai, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'GAB_YT'=>$gabYt, 'SAH_YT'=>$sahYt,
     ...
     * @throws Exception
     */
    public function update_kecacatan_calon ($putVars) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $constant = $this->constant;
            ...
            Class_db::getInstance()->db_update('ref_kecacatan_calon', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'KATEGORI'=>$kategori, 'JANTINA'=>$jantina, 'NILAI'=>$nilai, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'GAB_YT'=>$gabYt, 'SAH_YT'=>$sahYt,  'TARIKH_UBAHSUAI'=>'Now()'), array('KECACATAN_CALON_ID'=>$this->kecacatanCalonId));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
