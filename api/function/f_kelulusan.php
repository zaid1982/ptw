...
class Class_kelulusan {
     ...
     * @throws Exception
     */
    public function get_kelulusan_list () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            $result = array();
            $dbResults = Class_db::getInstance()->db_select('ref_kelulusan');
    ...
    public function get_kelulusan () {
        try {
            ...
            $result = array();
            $dbResult = Class_db::getInstance()->db_select_single('ref_kelulusan', array('KELULUSAN_ID'=>$this->kelulusanId), null, 1);
     ...
     * @throws Exception
     */
    public function add_kelulusan ($params=array()) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            if (Class_db::getInstance()->db_count('ref_kelulusan', array('KOD'=>$kod)) > 0) {
            ...
            return Class_db::getInstance()->db_insert('ref_kelulusan', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'GAB_YT'=>$gabYt, 'JENIS'=>$jenis, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'KATEGORI'=>$kategori, 'SAH_YT'=>$sahYt,
     ...
     * @throws Exception
     */
    public function update_kelulusan ($putVars) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $constant = $this->constant;
            ...
            if (Class_db::getInstance()->db_count('ref_kelulusan', array('KOD'=>$kod, 'KELULUSAN_ID'=>'<>'.$this->kelulusanId)) > 0) {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_KELULUSAN_KOD_EXIST, 31);
            }

            Class_db::getInstance()->db_update('ref_kelulusan', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'GAB_YT'=>$gabYt, 'JENIS'=>$jenis, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'KATEGORI'=>$kategori, 'SAH_YT'=>$sahYt,  'TARIKH_UBAHSUAI'=>'Now()'), array('KELULUSAN_ID'=>$this->kelulusanId));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
