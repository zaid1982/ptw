...
class Class_jenis_perkhidmatan {

    private $constant;
    private $fn_general;
    private $jenisPerkhidmatanId;
    private $userId;
     ...
     * @throws Exception
     */
    public function get_jenis_perkhidmatan_list () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            $result = array();
            $dbResults = Class_db::getInstance()->db_select('ref_jenis_perkhidmatan');
            foreach ($dbResults as $dbResult) {
                $rowResult['jenisPerkhidmatanId'] = $dbResult['JENIS_PERKHIDMATAN_ID'];
     ...
     * @throws Exception
     */
    public function get_jenis_perkhidmatan () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            if (empty($this->jenisPerkhidmatanId)) {
                throw new Exception('[' . __LINE__ . '] - Parameter jenisPerkhidmatanId empty');
            }

            $result = array();
            $dbResult = Class_db::getInstance()->db_select_single('ref_jenis_perkhidmatan', array('JENIS_PERKHIDMATAN_ID'=>$this->jenisPerkhidmatanId), null, 1);
            $result['jenisPerkhidmatanId'] = $dbResult['JENIS_PERKHIDMATAN_ID'];
     ...
     * @throws Exception
     */
    public function add_jenis_perkhidmatan ($params=array()) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            if (Class_db::getInstance()->db_count('ref_jenis_perkhidmatan', array('KOD'=>$kod)) > 0) {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_JENIS_PERKHIDMATAN_EXIST, 31);
            }

            $user = Class_db::getInstance()->db_select_single('sys_user', array('user_id'=>$this->userId), null, 1);
            return Class_db::getInstance()->db_insert('ref_jenis_perkhidmatan', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'KATEGORI'=>$kategori, 'JANTINA'=>$jantina, 'NILAI'=>$nilai, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'GAB_YT'=>$gabYt, 'SAH_YT'=>$sahYt,
     ...
     * @throws Exception
     */
    public function update_jenis_perkhidmatan ($putVars) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $constant = $this->constant;
            ...
            Class_db::getInstance()->db_update('ref_jenis_perkhidmatan', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'KATEGORI'=>$kategori, 'JANTINA'=>$jantina, 'NILAI'=>$nilai, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'GAB_YT'=>$gabYt, 'SAH_YT'=>$sahYt,  'TARIKH_UBAHSUAI'=>'Now()'), array('JENIS_PERKHIDMATAN_ID'=>$this->jenisPerkhidmatanId));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
