...
class Class_taraf_jawatan {

    private $constant;
    private $fn_general;
    private $tarafJawatanId;
    private $userId;
     ...
     * @throws Exception
     */
    public function get_taraf_jawatan_list () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            $result = array();
            $dbResults = Class_db::getInstance()->db_select('ref_taraf_jawatan');
            foreach ($dbResults as $dbResult) {
                $rowResult['tarafJawatanId'] = $dbResult['TARAF_JAWATAN_ID'];
     ...
     * @throws Exception
     */
    public function get_taraf_jawatan () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            if (empty($this->tarafJawatanId)) {
                throw new Exception('[' . __LINE__ . '] - Parameter tarafJawatanId empty');
            }

            $result = array();
            $dbResult = Class_db::getInstance()->db_select_single('ref_taraf_jawatan', array('TARAF_JAWATAN_ID'=>$this->tarafJawatanId), null, 1);
            $result['tarafJawatanId'] = $dbResult['TARAF_JAWATAN_ID'];
     ...
     * @throws Exception
     */
    public function add_taraf_jawatan ($params=array()) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            if (Class_db::getInstance()->db_count('ref_taraf_jawatan', array('KOD'=>$kod)) > 0) {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_TARAF_JAWATAN_EXIST, 31);
            }

            $user = Class_db::getInstance()->db_select_single('sys_user', array('user_id'=>$this->userId), null, 1);
            return Class_db::getInstance()->db_insert('ref_taraf_jawatan', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'KATEGORI'=>$kategori, 'JANTINA'=>$jantina, 'NILAI'=>$nilai, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'GAB_YT'=>$gabYt, 'SAH_YT'=>$sahYt,
     ...
     * @throws Exception
     */
    public function update_taraf_jawatan ($putVars) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $constant = $this->constant;
            ...
            Class_db::getInstance()->db_update('ref_taraf_jawatan', array('KOD'=>$kod, 'DISKRIPSI'=>$diskripsi, 'KATEGORI'=>$kategori, 'JANTINA'=>$jantina, 'NILAI'=>$nilai, 'NO_PEMEROLEHAN'=>$noPemerolehan, 'GAB_YT'=>$gabYt, 'SAH_YT'=>$sahYt,  'TARIKH_UBAHSUAI'=>'Now()'), array('TARAF_JAWATAN_ID'=>$this->tarafJawatanId));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
