...
class Class_calon_tkt3 {
     ...
     * @throws Exception
     */
    public function get_calon_tkt3_list () {
        try {
            ...
            $result = array();
            $dbResults = Class_db::getInstance()->db_select('calon_tkt3', array('id_pemohon'=>$this->idPemohon));
            foreach ($dbResults as $dbResult) {
                $rowResult['id'] = $dbResult['id'];
                $rowResult['idPemohon'] = $dbResult['id_pemohon'];
                $rowResult['jenisPeperiksaan'] = $this->fn_general->clear_null($dbResult['jenis_peperiksaan']);
                $rowResult['tahun'] = $this->fn_general->clear_null($dbResult['tahun']);
                $rowResult['mataPelajaran'] = $this->fn_general->clear_null($dbResult['mata_pelajaran']);
