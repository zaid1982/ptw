...
class Class_bakat_detail {
     ...
     * @throws Exception
     */
    public function get_bakat_detail_list () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            $result = array();
            $dbResults = Class_db::getInstance()->db_select('ref_bakat_detail');
            foreach ($dbResults as $dbResult) {
                $rowResult['kod'] = $dbResult['kod'];
                $rowResult['kodbakat'] = $dbResult['kodbakat'];
     ...
     * @throws Exception
     */
    public function get_bakat_detail () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            $result = array();
            $dbResult = Class_db::getInstance()->db_select_single('ref_bakat_detail', array('kod'=>$this->kod), null, 1);
            $result['kod'] = $dbResult['kod'];
            $result['kodbakat'] = $dbResult['kodbakat'];
     ...
     * @throws Exception
     */
    public function add_bakat_detail ($params=array()) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            if (!array_key_exists('kodbakat', $params) || empty($params['kodbakat'])) {
                throw new Exception('['.__LINE__.'] - Parameter kodbakat empty');
            }
            ...
            $kod = $params['kod'];
            $kodbakat = $params['kodbakat'];
            $diskripsi = $params['diskripsi'];
            $sahYt = $params['sahYt'];

            if (Class_db::getInstance()->db_count('ref_bakat_detail', array('kod'=>$kod)) > 0) {
            ...
            Class_db::getInstance()->db_insert('ref_bakat_detail', array('kod'=>$kod, 'kodbakat'=>$kodbakat, 'diskripsi'=>$diskripsi, 'sah_yt'=>$sahYt,
     ...
     * @throws Exception
     */
    public function update_bakat_detail ($putVars) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $constant = $this->constant;
            ...
            if (!isset($putVars['kodbakat']) || empty($putVars['kodbakat'])) {
                throw new Exception('[' . __LINE__ . '] - Parameter kodbakat empty');
            }
            ...
            $kodbakat = $putVars['kodbakat'];
            $diskripsi = $putVars['diskripsi'];
            $sahYt = $putVars['sahYt'];

            Class_db::getInstance()->db_update('ref_bakat_detail', array('kodbakat'=>$kodbakat, 'diskripsi'=>$diskripsi, 'sah_yt'=>$sahYt,  'd_ubahsuai'=>'Now()'), array('kod'=>$this->kod));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
