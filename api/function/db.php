...
class Class_db{
	...
	public function db_select_pdo($sqlstmt)
    {
        try { 
            if (!empty($this->DBH)){
                //$this->log_debug(__FUNCTION__, __LINE__, $sqlstmt);
                $stmt = $this->DBH->query($sqlstmt);
                $result = $stmt->fetchAll(PDO::FETCH_ASSOC);    
                $stmt = null;
    ...
    public function db_insert_pdo($sqlstmt)
    {
        try {    
            if (!empty($this->DBH)){
                //$this->log_debug(__FUNCTION__, __LINE__, $sqlstmt);
                $this->DBH->exec($sqlstmt);
	...
	public function db_connect_match() {
        try {
            $config = parse_ini_file('library/config.ini');
            $dbname = $config['dbname_match'];    
            $dbhost = $config['dbhost_match'];    
            $this->DBH = new PDO("mysql:host=$dbhost;dbname=$dbname;charset=utf8", $config['username_match'], $config['password_match']);
            $this->DBH->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
        }
        catch(PDOException $e) {
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $e->getMessage()));
