...
class Class_sijil {
     ...
     * @throws Exception
     */
    public function get_sijil_list () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            $result = array();
            $dbResults = Class_db::getInstance()->db_select('ref_sijil');
            foreach ($dbResults as $dbResult) {
                $rowResult['sijilId'] = $dbResult['SIJIL_ID'];
     ...
     * @throws Exception
     */
    public function get_sijil () {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);

            if (empty($this->sijilId)) {
                throw new Exception('[' . __LINE__ . '] - Parameter sijilId empty');
            }

            $result = array();
            $dbResult = Class_db::getInstance()->db_select_single('ref_sijil', array('SIJIL_ID'=>$this->sijilId), null, 1);
            $result['sijilId'] = $dbResult['SIJIL_ID'];
     ...
     * @throws Exception
     */
    public function add_sijil ($params=array()) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            ...
            if (Class_db::getInstance()->db_count('ref_sijil', array('KOD'=>$kod, 'TKT'=>$tkt)) > 0) {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_SIJIL_EXIST, 31);
            }

            return Class_db::getInstance()->db_insert('ref_sijil', array('KOD'=>$kod, 'TKT'=>$tkt, 'DISKRIPSI'=>$diskripsi, 'SAH_YT'=>$sahYt));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
     ...
     * @throws Exception
     */
    public function update_sijil ($putVars) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $constant = $this->constant;
            ...
            if (Class_db::getInstance()->db_count('ref_sijil', array('KOD'=>$kod, 'TKT'=>$tkt, 'SIJIL_ID'=>'<>'.$this->sijilId)) > 0) {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_SIJIL_EXIST, 31);
            }

            Class_db::getInstance()->db_update('ref_sijil', array('KOD'=>$kod, 'TKT'=>$tkt, 'DISKRIPSI'=>$diskripsi, 'SAH_YT'=>$sahYt), array('SIJIL_ID'=>$this->sijilId));
        }
        catch(Exception $ex) {
            $this->fn_general->log_error(__CLASS__, __FUNCTION__, __LINE__, $ex->getMessage());
            throw new Exception($this->get_exception('0005', __FUNCTION__, __LINE__, $ex->getMessage()), $ex->getCode());
