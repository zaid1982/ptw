...
const _ALERT_MSG_VALID_MYID = "Sila pastikan MyID diisi dengan format yang betul!";
const _ALERT_TITLE_ERROR = "RALAT";
const _ALERT_TITLE_WARNING = "AMARAN";
const _ALERT_MSG_ERROR_DEFAULT = "Ralat pada sistem. Sila hubungi pihak Administrator!";
const _ALERT_TITLE_SUCCESS = "BERJAYA";
const _ALERT_MSG_WARNING_NODATA = "Tiada data dipilih!";

const _ALERT_TITLE_SUCCESS_LOGOUT = "BERJAYA LOG KELUAR";
const _ALERT_MSG_SUCCESS_LOGOUT = "Anda berjaya log keluar.";
    ...
    const validateFields = function (field_id, validator, name, type) {
        ...
        $.each(validator, function (n2, u2) {
            if (!checkField(field_id, n2, u2)) {
                switch (n2) {
                    case 'notEmpty':
                        msg += '<br>Sila pastikan '+name+' diisi';
                        return false;
                    case 'eqLength':
                        msg += '<br>Sila pastikan panjang '+name+' bersamaan '+u2+' huruf';
                        break;
                    case 'maxLength':
                        msg += '<br>Sila pastikan panjang '+name+' tidak melebihi '+u2+' huruf';
                        break;
                    case 'minLength':
                        msg += '<br>Sila pastikan panjang '+name+' tidak kurang dari '+u2+' huruf';
                        break;
                    case 'numeric':
                        msg += '<br>Sila pastikan '+name+' dalam format nombor';
                        break;
                    case 'email':
                        msg += '<br>Sila pastikan '+name+' dalam format emel';
                        break;
                    case 'digit':
                        msg += '<br>Sila pastikan '+name+' dalam format digit sahaja';
                        break;
                    case 'similar':
                        msg += '<br>Sila pastikan '+name+' bersamaan dengan ' + u2.label;
                        break;
                    case 'max':
                        msg += '<br>Sila pastikan '+name+' tidak melebihi dari ' + u2;
                        break;
                    case 'min':
                        msg += '<br>Sila pastikan '+name+' tidak kurang dari ' + u2;
                        break;
                    case 'notEmptyArr':
                        msg += '<br>Sila pastikan '+name+' dipilih';
                        return false;
                    case 'notEmptyFile':
                        msg += '<br>Sila pastikan '+name+' dimuatnaik';
                        return false;
                    case 'notEmptyCheck':
...
function mzLogout() {
    mzAjaxRequest('user.php', 'POST', {action:'logout'});
...
function initiatePages() {
    ...
    $('.datepicker').pickadate({
        /*monthsFull: ['Januari', 'Februari', 'Mac', 'April', 'Mei', 'Jun', 'Julai', 'Ogos', 'September', 'Oktober',
            'November', 'Disember'],
        monthsShort: ['Jan', 'Feb', 'Mac', 'Apr', 'Mei', 'Jun', 'Jul', 'Ogo', 'Sep', 'Oct',
            'Nov', 'Dec'],
    ...
    $.each(menuSet, function (n, nav) {
        ...
        if (navSeconds.length > 0) {
            ...
            menuHtml += '<ul>';
            if (navId === nav['navId']) {
                titleHtml += '<span class="text-white">' + nav['navDesc'] + '</span>';
            }
...
function mzOptionStop(name, data, defaultText, keyIndex, valIndex, filters, type, isSort, valSort) {
    $('#'+name).materialSelect('destroy');
    mzOption(name, data, defaultText, keyIndex, valIndex, filters, type, isSort, valSort);
    $('#'+name).materialSelect();
}

function mzOption(name, data, defaultText, keyIndex, valIndex, filters, type, isSort, valSort) {
    ...
    if (isSort) {
        if (typeof valSort !== 'undefined' && valSort !== '') {
            dataSort.sort(function(a, b){
                return a[valSort].localeCompare(b[valSort]);
            });
        } else {
            dataSort.sort(function(a, b){
...
function mzGetMonthArray() {
    return [
        {monthId:1, monthName:'Januari'},
        {monthId:2, monthName:'Februari'},
        {monthId:3, monthName:'Mac'},
        {monthId:4, monthName:'April'},
        {monthId:5, monthName:'Mei'},
        {monthId:6, monthName:'Jun'},
        {monthId:7, monthName:'Julai'},
        {monthId:8, monthName:'Ogos'},
        {monthId:9, monthName:'September'},
        {monthId:10, monthName:'Oktober'},
        {monthId:11, monthName:'November'},
        {monthId:12, monthName:'Disember'}
    ];
}
